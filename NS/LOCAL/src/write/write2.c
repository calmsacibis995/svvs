...
setup("write");
funct("write");
testing ("Test write for a 2-way deadlock condition");
start
...
if ((pidB != 1) && (kill (pidB, 0) == 0))
	...
	/*
	 *  If this happens, look at the temp file ../tmp/dead1.dir/morelock.
	 *  If it contains 'hello', then both writes blocked.
	 *  If it contains 'healo', then the write in child A succeeded
	 *	(it should've failed with EDEADLK),
	 *	but the write in child B never woke up.
	 *  If it contains 'hbalo', then the write in child A succeeded
	 *	and the write in child B succeeded (whether it blocked or not).
	 *  It contains 'hbllo' if successful.
	 */
done ();
/*NOTREACHED*/
}
