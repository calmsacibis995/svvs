...
int len;
{
	...
	if (!len)
	{
		*str = '\0';
		if (!bad_exp(str, 0))
		{
			testing("Pattern match against: %s", &string[1]);
			...
			case 0:
				...
				testing("perform the system call");
				start
				expecting ( 0 );
				system(sbuf);
				finis
				(void) check_results(0, "sysout");
				break;
			default:
				expecting ( VOID );
				(void) unlink("sysout");
...
char *file;
{
	...
	{
		severr("Can not open temp file.");
	}

	expecting ( VOID );
	if((cnt = read(fd, buf, 10239))<0)
	{
		severr("Can not read temp file.");
	}
...
char *name;
{
	...
	if (*n || *p != '\n')
	{
		...
		*p = '\0';
		if (errcnt++ < 10)
			error("Pattern matching mismatch.\nThe expected pattern was %s.\nThe actual pattern was %s.\n", name, *pp);	/* ERROR */
		else
...
int len;
{
	...
	if (!len)
	{
		...
		*str = '\0';
		...
		{
			if((fd=open(temp_string, O_CREAT|O_WRONLY, 0664))<0)
			{
				(void) sprintf(xcept, "Can't creat temp file %s" , temp_string);
				severr(xcept);
			}
			expecting ( VOID );
			(void) close(fd);
