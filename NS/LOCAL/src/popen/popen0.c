...
char *rwtype;
{
	...
	if (!fptr)
	{
		error("Unexpected return status from popen.\nThe expected value was %x.\nThe actual value was %x.\n", ((char *)(!0)), fptr);	/*  ERROR */
}
}
...
char *line;
{
	...
	expecting ( VOID );
	if (fwrite(filbuf, 1, amt, fptr) != amt)
		severr("cannot fwrite to child process");
...
FILE *fptr;
{
	...
	while(fread(&fbuf[cnt], 1, 1, fptr)>0) cnt++;
	fbuf[cnt]='\0';
